...
{
    /// <summary> Limits the extent of a Point to the area where X and Y are at least 0 </summary>
    /// <param name="value">Point to be clamped</param>
    public static Point Clamp(this Point value) => new (Math.Max(value.X, 0), Math.Max(value.Y, 0));
    ...
    public static Point? FilterClamp(this Point value, double xMax, double yMax)
        => (value.X < 0 || value.X > xMax || value.Y < 0 || value.Y > yMax) ? (Point?)null : value;

    /// <summary> Limits the extent of a Point to the area between two points </summary>
    ...
    /// <summary> Return a Rect that specificed by two points and clipped by a rectangle specified by two other points. </summary>
    ...
    public static Rect Clip(Point value1, Point value2, Point topLeft, Point bottomRight)
    {
        ...
        return new(newTopLeft, size);
    }

    /// <summary> Moves and sized a border on a Canvas according to a Rect </summary>
